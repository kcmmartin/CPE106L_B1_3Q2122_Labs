...
class SavingsAccount:
    ...
    def __str__(self):
        ...
        self.list1.append(result)
        self.temp[self.pin] = result
        return result
...
def createBank(numAccounts = 1):
    ...
    for pinNumber in range(1000, upperPin):
        ...
        acct = SavingsAccount(name, str(pinNumber), balance)
        result =  'Name:    ' + acct.getName() + '\n' 
        result += 'PIN:     ' + acct.getPin() + '\n' 
        result += 'Balance: ' + str(acct.getBalance())
        list3.append(result)

    return "\n".join(map(str, sorted(list3)))
...
def testAccount():
    ...
    print("Expect 550:", account.getBalance())
    print(account.withdraw(100))
    print("Expect 450:", account.getBalance())
    print(account.withdraw(-50))
    print("Expect 500:", account.getBalance())
    print(account.withdraw(100000))
    print("Expect Insufficient Funds:", account.getBalance())
